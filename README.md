...
如有对项目感兴趣或有疑问，可以联系qq：2190448693

#### 一、项目地址及技术栈： <br>
>项目地址（账号：qaqqaq， 密码：qaqqaq，懒得注册可以用这个测试账号） <br>
  http://49.235.18.252/index <br> （因服务到期暂时无法使用） 
...
* 购票接口（/goupiao/UUID/do）
  ...
  * 第一层屏障**hasNoStockMap**本地标识 <br>
  ...
  本地标识每隔5s就会自动清理一次，也就是说，在5s之后该标识即消失，如果此时有大量访问，仍会访问到redis，如果此时redis中预减库存仍为负值，则本地缓存又会被重新写入（这样做是为了使退票之后，分布式部署的tomcat都能够即使知道有余票了） <br>
...
* 查询订单页面（order接口与order.html页面） <br>
  ...
  * 退票与**分布式锁** <br>
  退票时对于相应的三层标记都要清除哦！先写数据库，再改缓存很重要！（这里其实是有一些小问题的，但概率很小且不咋影响，就没有细处理了） <br>
  
  为了防止多用户退票情况下，redis预检库存的值与实际数据库不一致： <br>
  比如现在用户A和B都退票，这时候如果A先删除了数据库中的order，再查询数据库得到余票为n，然后B再删除数据库，查询得到余票为n+1，此时如果B先写缓存，如何A再写缓存，那么B查询的本应是正确的n+1值就会被A覆盖，实际上此时数据库中应有余票为n+1，但是缓存中为n，这就造成了缓存与数据库不一致，所以这里采用的分布式锁。 <br>
  具体分布式锁使用redis的set(NX, PX)实现的，没有此更为复杂的redis的分布式锁框架，具体可以自己去查一下 <br>
  [redis分布式锁实现](https://github.com/luopoQAQ/goupiao/blob/master/src/main/java/com/luopo/goupiao/redis/RedisService.java) <br>
  
  只有未出行的可以退票（发现order表的state字段没用上，其实order的state应该在这里标识的，而不是在前端简单判断一下，如果有幸你看到这里并且想完善可以私戳我） <br>
